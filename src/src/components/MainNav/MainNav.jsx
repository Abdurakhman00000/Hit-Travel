...
import { formatDateWithMonthName, formatDateWithoutYear } from '../utils/FormatDateWithoutYear'
...
const MainNav = ({
  ...
  useEffect(() => {
    if (dataFilter && dataFilter.regions && dataFilter.regions.region) {
      const data =
        regions && regions.regions && regions.regions.region && regions.regions.region
    ...
    if (countryData.name !== 'Выберите страну') {
      ...
      const datetoUrl =
        dateNightTo > new Date().toISOString().split('T')[0]
          ...
          : ''
      const nightsfromUrl = requests.budget[0] ? `&&nightsfrom=${requests.budget[0]}` : ''
      ...
      const childage1Url = count.count3 > 0 ? `&&childage1=${count.count3}` : ''
      const childage2Url = count.count4 > 0 ? `&&childage2=${count.count4}` : ''
      const childage3Url = count.count5 > 0 ? `&&childage3=${count.count5}` : ''
      ...
      const pricetoUrl = currencyModal.priceto
        ...
        : '&&priceto=999999999'
      const ratingUrl = rateHotelModal.ratecount ? `&&rating=${rateHotelModal.ratecount}` : ''
      const servicesUrl =
      ...
      const regionsUrl = resortModal.resort.length > 0 ? `&&regions=${resortModal.resortread}` : ''
      const subregionsUrl =
        resortModal.resirtchildrenread.length > 0
          ? `&&subregions=${resortModal.resirtchildrenread}`
          : ''
      const directOnlyUrl = `&&directOnly=${charter}`
      const hotelsUrl = hotelModal.hotel.length > 0 ? `&&hotels=${hotelModal.hotelread}` : ''
      const hoteltypesUrl =
      ...
      setLoader(true)
      if (depor.id) {
        navigate(`/city-and-country/${get}/${depor.name}/${countryData.name}/${hotelDetail}`)
      }
  ...
  console.log(requests, dateNightFrom, dateNightTo)
  console.log(tourHot)
    ...
    <div className="nav">
      ...
      <div className="container">
        <div className="nav_box">
          <div onClick={() => clickDiv('tour')} className={`nav_link ${tourDiv.tour && 'active'}`}>
            Поиск туров
          </div>
          <div
          ...
          <div onClick={() => clickDiv('hot')} className={`nav_link ${hotDiv.hot && 'active'}`}>
            Горящиe туры
          </div>
        </div>
      </div>
                    ...
                    {tourHot.map((el, id) => (
                      ...
                      >
                        ...
                        <div className="tour-info-wrap">
                          ...
                          <div className="like">
                            <p>
                              От <span className="old">{el.priceold}</span> {el.price} {el.currency}
                            </p>
                          </div>
                        </div>
                      </div>
      ...
      {hotelDiv.transition && (
        <div className={hotelDiv.hotel ? 'htkz' : ' htkz closed'}>
          ...
          <div className="container">
            ...
            <button onClick={() => Redirect({ hotel: true })} className="button_form">
              Найти отели
            </button>
          </div>
        </div>
            ...
            <div className="filt_new">
              <div className="filt_grid">
                ...
                >
                  ...
                  <h1>
                    {' '}
                    {formatDateWithoutYear(dateNightFrom)} - {formatDateWithoutYear(dateNightTo)}{' '}
                  </h1>
                </div>
                <div
                  onClick={() =>
              ...
              <div className="filt_grid">
                <div onClick={() => setUserModal(!userModal)} className="filt_box">
                  <p>Кто летит</p>
                  <h1>
                    {count.count1
                      ? count.count1 == 1
                ...
                <div onClick={() => setCharter(!charter)} className="filt_box prew">
                  <p>Только чартер</p>
                  <div style={{ display: 'flex', gap: 5 }}>
                    <input
                      type="checkbox"
                      checked={charter}
                      onChange={(e) => setCharter(e.target.value) || setCharter(!charter)}
                    />
              ...
              <div className="map_mest_travel">
                {filteredDatasCountry.map((el) => (
                  ...
                  >
                    ...
                    >
                      <p>{el.name}</p>
                      {el.name === countryData.name && <FaCheck size={24} color="#25d366" />}
                      {/* <span>
                      {el.directfly == 0
                        ? "Не прямой перелёт"
                        : "Прямой перелёт"}
